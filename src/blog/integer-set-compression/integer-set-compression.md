...
Since we're testing on synthetic data, we can now calculate the [Shannon entropy](https://en.wikipedia.org/wiki/Entropy_(information_theory)) of the data and use it as a lower bound!
...
The two contenders closest to the lower bound, in this case the [binary entropy function](https://en.wikipedia.org/wiki/Binary_entropy_function), are binary interpolative coding and a plain zlib-compressed bitset. The former does better when the density is farther from 50/50, and the latter does better as the density approaches 50/50 and we're trying to compress purely random data.
